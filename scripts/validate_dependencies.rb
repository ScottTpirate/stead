...
EXPECTED_SUCCESSOR_PGXPOOL_CLOSURE = {
  "github.com/jackc/pgpassfile" => ["v1.0.0", "h1:/6Hmqy13Ss2zCq62VdNG8tM1wchn8zjSGOBJ6icpsIM=", "h1:CEx0iS5ambNFdcRtxPj5JhEz+xB6uRky5eyVu/W2HEg=", "99d8e8e28945ffceaf75b0299fcb2bb656b8a683", "MIT", "adb1663fda031df8f4344aa68f299fd87d80353e31339406742ded21dae65702", "NOTICE-P1-015-PGPASSFILE-MIT"],
  "github.com/jackc/pgservicefile" => ["v0.0.0-20240606120523-5a60cdf6a761", "h1:iCEnooe7UlwOQYpKFhBabPMi4aNAfoODPEFNiAnClxo=", "h1:5TJZWKEWniPve33vlWYSoGYefn3gLQRzjfDlhSJ9ZKM=", "5a60cdf6a76120dc3d5152b95f3b5fd8aa7cc9eb", "MIT", "fc505773403fe869ed64cc2235cdd13988a427bb7e3a7e7004a3f4b27420f8fc", "NOTICE-P1-015-PGSERVICEFILE-MIT"],
  "github.com/jackc/puddle/v2" => ["v2.2.2", "h1:PR8nw+E/1w0GLuRFSmiioY6UooMp6KJv0/61nB7icHo=", "h1:vriiEXHvEE654aYKXXjOvZM39qJ0q+azkZFrfEOc3H4=", "bd09d14bd4018b6d65a9d7770e2f3ddf8b00af1c", "MIT", "2d50e98a4900b4d6457a38d39c1432fdc156fc2f7b365f2e33ec9344acbb0057", "NOTICE-P1-015-PUDDLE-MIT"],
  "golang.org/x/sync" => ["v0.22.0", "h1:SZjpbeLmrCk4xhRSZFNZW5gFUeCeFgjekvI/+gfScek=", "h1:9xrNwdLfx4jkKbNva9FpL6vEN7evnE43NNNJQ2LF3+0=", "1eb64d4bc0cde6da1bb8ebc7f178bb577508e5d0", "BSD-3-Clause", "911f8f5782931320f5b8d1160a76365b83aea6447ee6c04fa6d5591467db9dad", "NOTICE-P1-015-GO-X-SYNC-BSD-3-CLAUSE"],
  "golang.org/x/text" => ["v0.41.0", "h1:vz/seA0lnX87Othu2f/0L24RcgrXD9/YFTSuGjj3rH8=", "h1:jvf1O8ajNzZqhSrQBPbutR/EB83Cc0CFrezNQIwbb5M=", "acdba6655fd45cdb5ab73c9d6a8981333bd65a39", "BSD-3-Clause", "911f8f5782931320f5b8d1160a76365b83aea6447ee6c04fa6d5591467db9dad", "NOTICE-P1-015-GO-X-TEXT-BSD-3-CLAUSE"]
}.freeze
...
def postgresql_successor_errors(records_by_id, evidence)
  errors = []
  intake = evidence["successor_intake"] || {}
  go_evidence = intake["go_candidate"] || {}
  image_evidence = intake["oci_candidate"] || {}
  go_record = records_by_id[SUCCESSOR_PGX_APPROVAL_ID]
  image_record = records_by_id[SUCCESSOR_POSTGRES_APPROVAL_ID]

  errors << "dependency evidence: successor intake must remain pending and inactive" unless intake.values_at("state", "release_eligible", "activation_allowed") == ["REVIEWED_PENDING_INDEPENDENT_APPROVAL_NOT_INTEGRATED", false, false]
  errors << "dependency evidence: pgx successor class or state differs from intake" unless go_evidence.values_at("approval_id", "classification", "review_state", "activation_allowed", "release_eligible") == [SUCCESSOR_PGX_APPROVAL_ID, "PRODUCTION_RUNTIME_DISTRIBUTED", "REVIEWED_PENDING_INDEPENDENT_APPROVAL", false, false]
  errors << "dependency evidence: image successor class or state differs from intake" unless image_evidence.values_at("approval_id", "classification", "review_state", "activation_allowed", "distribution_allowed") == [SUCCESSOR_POSTGRES_APPROVAL_ID, "DEVELOPMENT_CI_ONLY_NON_DISTRIBUTED", "REVIEWED_PENDING_INDEPENDENT_APPROVAL", false, false]
  closure = Array(go_evidence["pgxpool_runtime_closure"])
  actual_closure = closure.to_h do |entry|
    [entry["module"], entry.values_at("version", "module_sum", "go_mod_sum", "upstream_commit", "license_expression", "license_sha256", "notice_id")]
  end
  errors << "dependency evidence: successor pgxpool closure differs from the approval-ID-bound intake" unless closure.length == 5 && actual_closure == EXPECTED_SUCCESSOR_PGXPOOL_CLOSURE
  go_scans = Array(go_evidence["vulnerability_scans"])
  errors << "dependency evidence: successor pgx scans are incomplete or nonzero" unless go_scans.length == 2 && go_scans.all? { |scan| scan["findings"] == 0 && scan["report_sha256"]&.match?(/\A[0-9a-f]{64}\z/) && scan.dig("tool", "database_updated_at") }

  image_scans = Array(image_evidence["vulnerability_scans"])
  trivy = image_scans.find { |scan| scan["tool"] == "trivy" } || {}
  grype = image_scans.find { |scan| scan["tool"] == "grype" } || {}
  errors << "dependency evidence: image scan summary or contextual Medium differs from intake" unless [trivy.dig("findings", "critical"), trivy.dig("findings", "high"), grype.dig("findings", "critical"), grype.dig("findings", "high"), grype.dig("findings", "medium"), grype.dig("contextual_medium", "id")] == [0, 0, 0, 0, 1, "CVE-2026-18374"]
  errors << "dependency evidence: image signatures and attestations are not verified" unless image_evidence.dig("signature_and_attestations", "status") == "VERIFIED_KEYLESS_SIGNATURES_AND_ATTESTATIONS"
  inventory = image_evidence["package_license_inventory"] || {}
  apk_packages = inventory["installed_apk_packages"] || {}
  embedded_modules = inventory["embedded_go_modules"] || []
  errors << "dependency evidence: exact image license inventory is incomplete" unless apk_packages.length == 56 && embedded_modules.length == 4 && (apk_packages.values + embedded_modules).all? { |entry| entry.is_a?(Hash) && !entry["version"].to_s.empty? && !entry["license_expression"].to_s.empty? }

  candidates = [[SUCCESSOR_PGX_APPROVAL_ID, go_record, go_evidence, ["github.com/jackc/pgx/v5", "go_module", "go", "runtime", ["stead-api"], "ALLOW-PERMISSIVE"]], [SUCCESSOR_POSTGRES_APPROVAL_ID, image_record, image_evidence, ["cgr.dev/chainguard/postgres", "oci_image", "oci", "test", [], "REVIEW-NONRUNTIME"]]]
  candidates.each do |approval_id, record, candidate_evidence, boundary|
    if record.nil?
      errors << "dependency registry: missing successor candidate #{approval_id}"
      next
    end
    actual_boundary = [record.dig("component", "name"), record.dig("component", "kind"), record.dig("component", "ecosystem"), record.dig("usage", "relationship"), record.dig("usage", "distributed_in"), record.dig("decision", "category")]
    errors << "#{approval_id}: registry bounded-use classification differs from successor evidence" unless actual_boundary == boundary
    errors << "#{approval_id}: registry version differs from successor evidence" unless record.dig("component", "version") == candidate_evidence["version"]
    expected_digest = approval_id == SUCCESSOR_PGX_APPROVAL_ID ? candidate_evidence["module_sum"] : candidate_evidence["index_digest"]
    errors << "#{approval_id}: registry digest differs from successor evidence" unless record.dig("component", "digest", "value") == expected_digest

    approval = evidence.dig("successor_approval", "candidates", approval_id)
    if record.dig("decision", "status") == "APPROVED"
      if approval.nil?
        errors << "#{approval_id}: APPROVED requires exact-revision successor_approval evidence"
        next
      end
      errors << "#{approval_id}: approval evidence must bind a 40-character candidate revision" unless approval["candidate_revision"]&.match?(/\A[0-9a-f]{40}\z/)
      errors << "#{approval_id}: approval evidence identities differ from the registry" unless approval["approvers"] == record.dig("decision", "approvers")
      errors << "#{approval_id}: approval evidence timestamp differs from the registry" unless approval["approved_at"] == record.dig("decision", "approved_at")
      errors << "#{approval_id}: approval evidence must authorize exact activation" unless approval["activation_allowed"] == true
      errors << "#{approval_id}: exact candidate rescan must pass before approval" unless approval.dig("rescan", "status") == "PASS"
      test_key = approval_id == SUCCESSOR_PGX_APPROVAL_ID ? "compatibility_tests" : "functional_test"
      errors << "#{approval_id}: #{test_key} must pass before approval" unless approval.dig(test_key, "status") == "PASS"
    elsif approval
      errors << "#{approval_id}: successor_approval evidence is not valid while the registry record is #{record.dig('decision', 'status')}"
    end
  end
  errors << "#{SUCCESSOR_PGX_APPROVAL_ID}: registry go.mod checksum differs from successor evidence" if go_record && go_record.dig("component", "module_file_digest", "value") != go_evidence["go_mod_sum"]
  errors << "#{SUCCESSOR_PGX_APPROVAL_ID}: successor notice obligations differ from closure evidence" if go_record && go_record.dig("obligations", "notices") != SUCCESSOR_PGX_NOTICE_IDS
  errors << "dependency evidence: pending image intake may not claim a completed functional test" unless image_evidence.dig("functional_test", "status") == "PENDING_REQUIRED_BEFORE_APPROVAL"
  errors
end
...
def run_validator_self_tests
  ...
  unless missing_approval_evidence_errors.any? { |error| error.include?("requires exact-revision successor_approval evidence") }
  ...
  validate_go_dependencies(
    ...
    incomplete_closure_errors,
    go_closures_by_approval_id: { SUCCESSOR_PGX_APPROVAL_ID => EXPECTED_SUCCESSOR_PGXPOOL_CLOSURE.transform_values { |values| values.first(3) } },
...
go_closures_by_approval_id = {
  SUCCESSOR_PGX_APPROVAL_ID => EXPECTED_SUCCESSOR_PGXPOOL_CLOSURE.transform_values { |values| values.first(3) }
}
