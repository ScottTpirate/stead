...
The inactive provider-effect store has also passed a [fresh real PostgreSQL storage test](docs/governance/dependency-evidence/postgresql-effect-storage-proof-20260905.json): atomic effect/audit/outbox rollback, competing updates, and fail-closed revocation fences under malformed records and inspection timeout. A concrete hidden-tracker adapter now has reviewed source-level tests connecting issued and consumed effect execution to one bounded HTTP operation. These use owned test endpoints, not the running Gitea service. Protected plan persistence, provider proof, final validation and atomic canonical acceptance remain unconnected; neither result activates a provider capability.
...
Fresh product testing at exact application/harness `daea3d49280dbc2b72c8271f010d6d744d864d88` passed supported startup, service smoke and four fresh instance-bound signed checks. Its real HTTPS browser run completed login, Organization creation/read, parent and child Team creation/read, general Project creation/read, and reload/Refresh reads. It then failed the command-palette keyboard stage; the distinct no-grant principal was not exercised. Five reached axe surfaces reported no violations, but contrast and palette ARIA checks remain incomplete. Independent QA/security verified the partial result, unchanged inputs/runtime and observable cleanup. A separately admitted anonymous keyboard diagnostic against that same application passed all 14 steps with harness `f2cbd2016346fa1d767bea3a9ea833efb466970b`; it did not reproduce or fix the authenticated failure. Reviewed journey instrumentation now records the first failed keyboard substep without relaxing assertions. This installation is stopped, with its original state and independently verified private snapshot retained. This is not Checkpoint A acceptance, independent SQL/audit correlation or a performance-gate pass.

The next separate fresh installation at `153ba414436faac86c587857198ce5232f02c5be` failed closed before activation or browser testing. Its actual policy conformance passed 32/32 rows, but the mutation campaign stopped after 28 completed cases with invalid evidence at guard-032; it did not complete the required 111-case campaign. An isolated cold-cache positive-control/guard-032 diagnostic subsequently passed and produced a genuine test kill, so the original failure's cause remains unknown. The failed installation, first outputs and partial state are stopped and preserved with independently verified snapshot integrity. Final source foundation/dev checks passed only on explicitly recorded corrected test-environment reruns; they do not turn the failed installation into a pass. Reviewed source corrections now retain bounded compiler-event diagnostics and distinguish pending displayed collections from settled empty/error results. They do not establish the original failure's cause or activate another candidate.

The later exact candidate `fdb7187a9644f52f7bf3ee7f3be7c6c70d2f61aa` passed fresh startup, seven-service smoke and all four actual instance-bound checks (32/32 conformance, 104/111 mutations, 3/3 dependencies, 1/1 offline archive). Its first real HTTPS browser attempt completed seven stages, including all 22 keyboard substeps, then failed at the no-grant collection stage. Independent API observations confirm session200 followed by Organization-list404: the harness expected empty200, but the principal lacks the existing collection permission. That denied list also reported zero audit writes, exposing a separate missing aggregate denial-record path. Both sessions and the failed attempt remain private and intact. Five reached axe surfaces retain serious contrast/ARIA incompletes; neither full accessibility nor the seven-surface CSP aggregate passed. Independent QA/security verified unchanged runtime/inputs and observable cleanup. This installation is now stopped through its supported path, with all data, keys/anchor, binaries and served assets retained in the original state and a consistent checked snapshot. Independently reviewed source corrections now distinguish denied collections from empty success, audit required scope/row denials without per-hidden-row writes, give palette results a valid named group, and enable actual Unicode glyph contrast calculation without disabling rules. A new exact-source installation/workload must verify them in the product; this failed run is not made reusable or relabeled as a pass.

The first integrated source check at `aa087ecd53912fa1311464c4eb37f58c0dfcd564` stopped before any new activation: its mutation selector missed the moved authorization implementation, reducing the inventory to 88. That failed attempt is preserved, not accepted. The focused selector correction restores all 111 prior guards and adds nine collection guards, includes their behavioral tests in the campaign, and rejects missing selected functions. Its actual 120-case campaign must pass the unchanged 90-percent floor and receive new exact-source review; earlier mutation results do not transfer.

The bounded Commonplace helper observation has run against unchanged upstream `a6491e34e4de933bd88c90e1a251c1566366cf7b`, collecting 19 document and four log cases with independent QA/security verification. Ordinary metadata roundtrips worked, but malformed-input handling, type validation, whitespace preservation and chronological log insertion exposed gaps. This private, isolated helper scope does not approve dependency distribution or establish Gitea/headless/editor compatibility. [STEAD-P1-004](https://github.com/ScottTpirate/stead/issues/9) retains the remaining compatibility work; no native Docs fallback or complete Docs implementation has been accepted.
