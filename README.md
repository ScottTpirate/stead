...
Phase 0 is approved and baselined at tag `phase0`, commit `e24a4d9d05ad6df19c5bcaa9c385ee74fd5d8c31`. Phase 1 implementation is active: ADR-0001 through ADR-0009 are accepted; `STEAD-P1-001` and `STEAD-P1-016` are complete; and the original Stead frontend foundation is merged. ADR-0009 is accepted at `b64384249a82f6f744ec07a002f70de6e24e15e6`. The persistent [Phase 1 integration PR](https://github.com/ScottTpirate/stead/pull/45) now contains an approved PostgreSQL runtime driver, real database transactions, local identity and authorization consumers, and Organization/Team/Project API and UI implementation. Live database role isolation, session exchange/revocation, atomic audit/outbox transactions, and OpenFGA protocol tests pass. The first real browser journey now creates and reads an Organization, parent/child Teams, and a general Project in PostgreSQL, with each mutation's domain, audit, idempotency, and outbox records committed together. A transient parallel-read failure discovered on reload is under regression and fresh-installation verification; Checkpoint A reliability and performance gates are not yet complete. The owner-approved local bootstrap amendment is merged in [PR #46](https://github.com/ScottTpirate/stead/pull/46); its reviewed signed derivation implementation has completed real per-installation policy, mutation, dependency, and offline verification checks. Development service dependencies have [distinct scoped approval](deploy/compose/dev-service-review.md), limited to the documented synthetic use. Collection authorization uses set-oriented SQL and fresh OpenFGA batch checks. Live isolated OpenFGA tests confirm that Team hierarchy and ownership do not confer access; the first browser journey alone does not prove denied-principal behavior. Outbox intents persist but asynchronous delivery, Work/Docs integration, and the full release gates remain unfinished. Neither `STEAD-P1-015` nor `STEAD-P1-005` is complete. `STEAD-P1-017` is deferred until the first Phase 2 migration implementation actually needs its namespace; Phase 2–3 otherwise remain phase-gated.
