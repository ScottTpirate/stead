...
var schemaTables = map[string]map[string]string{
	...
	"authorization": {
		...
		"effects":        `id uuid PRIMARY KEY, operation_id uuid NOT NULL UNIQUE, project_id uuid NOT NULL UNIQUE, session_id uuid NOT NULL, state text NOT NULL CHECK(state IN ('issued','consumed','reconciling','terminal')), version bigint NOT NULL CHECK(version>0), record jsonb NOT NULL, label jsonb NOT NULL, UNIQUE(session_id,id)`,
