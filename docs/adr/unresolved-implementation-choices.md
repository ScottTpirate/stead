...
**Status:** Active candidate queue; `ADR-CAND-001` is resolved, five first-slice decisions are proposed for approval, and the remaining entries are deferred to their named decision point<br>
...
| Candidate | Proposed record | Approval state |
|---|---|---|
| `ADR-CAND-003` Authorization and policy-decision topology | [ADR-0005](./0005-authorization-and-policy-decision-topology.md) selects the native in-process Go evaluator, a single fail-closed coordinator, zero decision caching, bounded deadlines, revision fencing, durable audit preconditions, and short-lived provider credentials. | Required non-author reviews are being recorded; project-owner approval is not required for this conforming implementation choice. |
| `ADR-CAND-004` Security-label algebra and profile identifiers | [ADR-0002](./0002-security-label-algebra-and-profile-identifiers.md) selects a product partial order, conservative joins, stable commercial/US-government profile identifiers, distinct label/profile/bundle revisions, and deny-on-unrepresentable composition. | Pending explicit project-owner approval of controlled label/profile contracts after required reviews. |
| `ADR-CAND-005` Trusted principal and runtime-attribute normalization | [ADR-0003](./0003-trusted-principal-and-runtime-attributes.md) selects typed authority-bound assertions, deterministic precedence/conflict/freshness rules, immutable OIDC/SCIM correlation, revision fencing, and task/runtime-bound Agent evidence. | Pending explicit project-owner approval of the pre-consumer closed `POL-DECISION-IO-V0.1` correction after required reviews. |
| `ADR-CAND-007` Policy-bundle distribution and trust roots | [ADR-0006](./0006-signed-policy-bundle-distribution-and-activation.md) selects the Stead Policy Activation Set v1, reproducible unsigned content, DSSE/P-256 signing, offline trust rotation, and atomic OpenFGA/policy activation. | Pending explicit project-owner approval of the signing-format and controlled activation contracts after required reviews. |
| `ADR-CAND-021` Initial Team relation model | [ADR-0004](./0004-initial-team-role-and-authorization-semantics.md) selects fixed explicit `lead`, `member`, and `contributor` relations while preserving zero hierarchy/accountability inheritance. | Pending explicit project-owner approval of the public Team-role and authorization contract after required reviews. |
