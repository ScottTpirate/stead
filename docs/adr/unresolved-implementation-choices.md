...
|---|---|---|
| `ADR-CAND-006` Event tenant/domain partition and replay contract | [ADR-0008](./0008-nats-stream-subject-retention-replay-ordering-and-dlq.md) proposes one internal Stead application account and two fixed streams per deployment security domain, the locked event subjects, service-role credentials, no Organization-time broker provisioning, PostgreSQL consumer idempotency, minimized dead letters, authorized replay, and owner-snapshot projection rebuild. | WS-07 ownership plus WS-01, WS-02, WS-06, WS-08, WS-12, and distinct independent WS-13 QA/security reviews must accept one immutable revision before the early WS-12 development-stack/config child or `STEAD-P1-007` activates. Full P1-011 remains downstream of P1-007. Per-Organization accounts remain a later evidence-gated option. Project-owner approval is not required unless review changes a locked or project-owner-controlled contract. |
