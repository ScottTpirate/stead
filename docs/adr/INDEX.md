...
| Proposed | [ADR-0007: PostgreSQL module isolation and transaction coordination](./0007-postgresql-module-isolation-and-transaction-coordination.md), proposed for `ADR-CAND-002`; it remains blocking until accepted at an immutable revision with the required non-author reviews. |
| Rejected | None. |
| Deferred | `ADR-CAND-006` and `ADR-CAND-008`–`ADR-CAND-020` in [unresolved-implementation-choices.md](./unresolved-implementation-choices.md), each with a decision owner, reviewers, fixed constraints, and a deadline before dependent implementation. |
