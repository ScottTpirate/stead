...
| Proposed | [ADR-0008: NATS stream, subject, retention, replay, ordering, and dead-letter contract](./0008-nats-stream-subject-retention-replay-ordering-and-dlq.md), proposed for `ADR-CAND-006`; it blocks the `STEAD-P1-015` recovery-port sub-slice and NATS-dependent work until accepted at an immutable revision with the required non-author reviews. |
| Rejected | None. |
| Deferred | `ADR-CAND-008`–`ADR-CAND-020` in [unresolved-implementation-choices.md](./unresolved-implementation-choices.md), each with a decision owner, reviewers, fixed constraints, and a deadline before dependent implementation. |
