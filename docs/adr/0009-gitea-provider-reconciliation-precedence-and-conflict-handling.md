...
- `ProviderOperationID`: a Stead UUIDv7 bound to the caller idempotency key, canonical operation digest, resource/fence snapshot, and a closed compatibility-profile call plan; every actual call attempt under that operation binds its own one-use authorization-effect permit and ordinal;
...
6. Select exactly one outcome for the resource generation:
   ...
   - `reset`: create or resume one durable, fenced provider operation for the exact declared state, apply the minimum documented provider operation, read/verify the result, and atomically persist the confirmed projection plus audit/outbox intent; each write, retry, and verification read repeats steps 3 and 4 with a distinct call-bound permit;
...
1. **Prepare:** bind the exact consistency vector, provider snapshot token, and closed compatibility-profile call plan; insert the WS-03 operation intent and append its audit intent through owned ports. Freshly run the complete decision sequence and issue a call-bound `AuthorizationEffectPermit` only for the first actual provider request in that plan.
2. **Execute:** immediately before each preflight read, mutation, retry, follow-up page, or verification read, freshly authorize that exact call/action and scope, commit its own one-use permit plus audit/outbox intent, recheck the fence, atomically consume only that permit, and invoke the provider once. A prior read or mutation permit cannot authorize the next call. A strong-capability operation normally uses one mutation call and at most one verification read. A weak-capability operation may use one preflight read, one mutation, and one verification read. The compatibility profile fixes the bound and each call ordinal; a denied or changed decision stops before that call without erasing any earlier ambiguous effect.
3. **Confirm:** only an authoritative success representation or a separately permitted verified read matching the exact intended normalized result confirms the effect. A transaction records provider identity/token, immediate local projection, operation terminal result, the ordered call/permit outcomes, and outbox/audit evidence before returning success. The response never waits for NATS or a projection consumer.
...
| `T-ADR-0009-AMBIGUOUS-MUTATION` | Lost responses for update/delete/create exercise before/after/third-state, apply/revert, delete/recreate, delayed true create, and coincident zero/one/multiple-candidate states. No snapshot alone or absence-after-horizon can terminalize success or `failed_without_effect`; create adoption requires a verified operation-bound immutable key. Preflight, mutation, retry/search page, and verification calls each require a distinct current decision and call-bound permit; no blind retry, duplicate, cross-call permit reuse, unaudited call, ABA terminalization, candidate misadoption, or false success occurs. |
