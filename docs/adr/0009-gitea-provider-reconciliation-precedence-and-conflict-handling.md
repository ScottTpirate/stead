...
The webhook receiver accepts only the configured `POST` plus `application/json` profile and verifies a non-empty `X-Gitea-Signature` as HMAC-SHA256 over the exact bounded raw body, using constant-time comparison, before parsing. The v1 receiver ceiling is 4 MiB of raw body and JSON nesting depth 32; a compatibility profile may lower but not raise either limit. It checks installation/hook identity and active secret epoch, event and exact event type, content type, and the attempt UUID. Repeating the same attempt key and body digest is a no-op; the same key with another digest is a security incident and quarantines the hook. A redelivery with a different attempt UUID remains harmless because the receiver never applies the payload as a canonical delta: it durably marks the mapped provider resource dirty and schedules a fresh snapshot reconciliation. Unsupported, unsigned, malformed, unmapped, or over-limit deliveries cannot mutate a canonical owner; scheduled reconciliation remains the missed-event recovery path.
...
One resource reconciliation serializes through a WS-03-owned optimistic lease/generation; it does not hold a database transaction during Gitea I/O. It performs:

1. Resolve the installation capability profile and opaque resource mapping. Missing, colliding, reused, or cross-scope mappings quarantine.
2. Read the current provider snapshot with a bounded deadline/page size and normalize it under that exact profile.
3. Load the current declared/last-confirmed Stead state, provider-enforcement fence, pending provider operation, and prior snapshot token through owned ports.
4. Classify every difference by the closed table. A webhook sender is only untrusted evidence until it maps uniquely to a current principal and action. Direct-change acceptance requires a fresh central authorization/classification decision against current state; historical or provider-local permission cannot substitute.
5. Select exactly one outcome for the resource generation:
...
6. Re-read the local generation/fence before completion. A concurrent canonical change, security change, or newer dirty generation makes the result stale and schedules another bounded pass; it never overwrites newer state.
...
For ambiguous update/delete operations, reconciliation compares the verified provider snapshot with the exact before and intended-after snapshots: after means confirm; before means proven no-effect; any third state quarantines as a concurrent conflict. For an ambiguous create, the adapter searches only the bounded operation window and expected parent using service actor, resource kind, normalized content digest, and immutable provider fields. Exactly one exact candidate may be adopted; none after the compatibility profile's visibility horizon proves no-effect; multiple or unverifiable candidates quarantine. Non-idempotent creates are never blindly replayed. Manual recovery chooses among already observed candidates or records no-effect; it cannot fabricate success, silently delete user content, or bypass the current authorization/fence check.

Only terminal proof changes an ADR-0005 permit from `reconciling` to `terminal`. A queued retry does not extend or reuse a permit. A reset/repair is a new authorized operation with its own idempotency key and permit.

### Scheduling, outage, rate limiting, and degraded behavior

Three paths share the same reconciliation function:
...
| `T-ADR-0009-AMBIGUOUS-MUTATION` | Lost responses for update/delete/create exercise before/after/third-state and zero/one/multiple-candidate rules; no blind retry, duplicate, permit reuse, or false success occurs. |
| `T-ADR-0009-FULL-RECONCILIATION` | Bounded pagination, schedule jitter, duplicate/out-of-order pages, provider mutation during scan, cursor restart, webhook race, and final generation recheck converge to the same result with measured call/query/memory bounds. |
