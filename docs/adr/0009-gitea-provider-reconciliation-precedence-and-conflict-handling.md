...
The only eligible calls are declared pagination reads, snapshot reads, bounded verification reads, and compatibility-profile-declared safe retries of idempotent reads. Every dispatched or uncertain read consumes its local allowance. The operation performs one atomic start/claim transaction and one terminal transaction that appends exactly one logical audit record, but zero durable reservation, permit, audit-record, claim-renewal, or accounting writes per eligible call or page. The start transaction persists only an operation-state audit-intent reference; it does not create or mutate an append-only audit record. Clean completion records exact counts; interruption or expiry recovery conservatively records the reserved upper bound and an abandoned/crash outcome. Scope, claim, accounting, and logical-audit control writes therefore remain constant as page count grows. Batched projection, domain, outbox, and quarantine writes are measured separately and may grow only with actual changed resources.
...
- WS-01 owns the versioned canonical `AuditRecord` schema; WS-07 owns its audit-store consumption, the versioned event/dead-letter schemas, schema registration, AsyncAPI, and delivery. NATS is never a request-path correctness dependency.
...
Every performance-sensitive implementation reports endpoint/scenario, p50/p95/p99, SQL query and PostgreSQL write counts, OpenFGA and provider calls, response size, frontend chunk impact, and baseline delta. Ordinary UI reads assert zero provider calls. Full scans assert bounded memory/calls and constant scope/claim/accounting/logical-audit control writes as page count grows; changed-resource projection, domain, outbox, and quarantine writes remain batched and set-oriented and are reported separately.
...
| `T-ADR-0009-FULL-RECONCILIATION` | Every scope binding and bound has negative coverage; an atomic one-holder claim blocks same-scope forks, stale holders, expiry, and post-terminal replay; inventory cannot widen scope; process loss starts fresh; eligible pages add no control writes; scope/claim/accounting/logical-audit control writes remain constant as pages grow while changed-resource writes are measured separately. |
| `T-ADR-0009-AUDIT-MINIMIZATION` | Canonical audit/event/outbox/DLQ composition, closed nested provider evidence and operational-surface attribute profiles, opaque non-content-derived audit references, offline-guessing/cross-event provider-plan correlation checks, and the full forbidden-value-by-base-envelope/nested-evidence/surface canary matrix prove that raw provider plans, paths, resources, cursors, payloads, headers, secrets, credentials, protected content, and exception text never propagate. |
