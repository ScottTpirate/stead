...
- `ProviderAuthorizationScope`: one fresh central decision for one bounded logical reconciliation or provider operation, binding the acting service principal, initiating principal when relevant, Organization/security domain, canonical resource/container scope, allowed provider operation class, closed call plan, activation and authorization revisions, expiry, and provider compatibility profile;
...
One resource reconciliation serializes through a WS-03-owned optimistic lease/generation; it does not hold a database transaction during Gitea I/O. Internal maintenance, webhook, full-scan, and recovery paths are not exempt from ADR-0005. Before provider I/O, the worker obtains one fresh `ProviderAuthorizationScope` for the bounded logical operation. The scope's closed call plan fixes endpoint/operation classes, maximum calls/pages/items/bytes, cursor rules, safe idempotent-read retries, deadline, and expiry. It cannot be widened or renewed from provider output. Provider read pagination, snapshot reads, bounded verification reads, and safe retry of an idempotent read under that scope do not each create a durable `AuthorizationEffectPermit`. The worker records one logical audit operation with actual call counts, page/item counts, safe result classes, fence outcomes, and terminal status. The worker performs:
...
3. Authenticate the acting service principal and initiating principal when present, then run the complete current central authorization/classification and provider/path sequence for the exact `ProviderAuthorizationScope`. Persist the bounded scope and logical audit start when the job is durable. Missing, stale, pending, mismatched, expired, or denied context stops before network I/O.
4. Before each provider read, validate its operation class, resource/container, ordinal, cursor, deadline, and cumulative call/page/item/byte bounds against the scope. Recheck the latest activation, authorization, provider-enforcement, and resource fences wherever security-sensitive data could be accepted. Normalize only a verified response under the bound compatibility profile. A timeout or malformed response may retry only when the profile declares the read idempotent and the original bounds still hold; it never expands scope or becomes unaudited.
...
7. Re-read the local generation and all bound revisions/fences before completion. A concurrent canonical change, security change, newer dirty generation, expired scope, or exceeded call plan makes the result stale and schedules another freshly authorized bounded pass; it never overwrites newer state.
...
2. **Execute:** validate every read against the scope and its cumulative bounds. Immediately before a provider mutation, credential issuance, direct Git/protocol access, export/download, non-idempotent call, ambiguous external effect, or operation that can outlive the logical request/job, freshly authorize that effect, commit its own one-use `AuthorizationEffectPermit`, recheck the latest fences, atomically consume only that permit, and invoke the effect once. Safe idempotent read retries and bounded verification reads remain inside the original scope; a mutation permit never authorizes another call or widens the read plan.
...
Only terminal proof changes an ADR-0005 permit from `reconciling` to `terminal`. A queued retry does not extend or reuse a permit. Reconciliation reads and pages stay within one unexpanded `ProviderAuthorizationScope`; the original mutation permit remains `reconciling`, and a reset/repair is a new authorized operation with its own idempotency key and permit.
...
| `T-ADR-0009-AMBIGUOUS-MUTATION` | Lost responses for update/delete/create exercise before/after/third-state, apply/revert, delete/recreate, delayed true create, and coincident zero/one/multiple-candidate states. No snapshot alone or absence-after-horizon can terminalize success or `failed_without_effect`; create adoption requires a verified operation-bound immutable key. The bounded read plan cannot widen, every mutation/non-idempotent or ambiguous effect requires a fresh one-use permit, final fences are checked before canonical acceptance, and no blind retry, duplicate, ABA terminalization, candidate misadoption, or false success occurs. |
| `T-ADR-0009-FULL-RECONCILIATION` | One fresh `ProviderAuthorizationScope` binds service/initiating principals, domain, container/resource, operation class, call/page/item/byte limits, revisions, expiry, and compatibility profile. Bounded pagination, snapshot/verification reads, and safe idempotent-read retry use it without per-page permits; scope validation, latest fences, denial-before-I/O, one logical audit with actual counts/results, and final generation recheck converge with measured call/query/memory bounds. |
