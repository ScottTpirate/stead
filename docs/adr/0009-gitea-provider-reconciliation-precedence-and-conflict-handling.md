...
5. Classify every difference by the closed table. A webhook sender is only evidence until it maps uniquely to a current principal and action. Direct-change acceptance runs a separate complete fresh central decision for that acting principal and exact canonical mutation against the current post-read state, then rechecks the bound fence inside the owning transaction before changing canonical state; the service actor's read permit, historical permission, and provider-local permission cannot authorize acceptance.
...
| `T-ADR-0009-AMBIGUOUS-MUTATION` | Lost responses for update/delete/create exercise before/after/third-state and zero/one/multiple-candidate rules; absence-after-horizon cannot produce `failed_without_effect`; preflight, mutation, retry/search page, and verification calls each require a distinct current decision and call-bound permit; no blind retry, duplicate, cross-call permit reuse, unaudited call, or false success occurs. |
