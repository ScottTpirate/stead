...
- **Requirement IDs:** `PRIN-002`, `ARCH-004`, `DOM-004`, `SCM-001`, `SCM-002`, `SCM-003`, `SCM-004`, `SCM-005`, `AUTH-002`, `AUTH-006`, `CLS-003`, `CLS-006`, `CLS-007`, `TEST-006`, `PERF-003`, `PERF-004`
...
For one reconciliation or provider operation, WS-06 issues one fresh, unique, nonrenewable, nontransferable `ProviderAuthorizationScope`. It binds the acting service principal; initiating principal or explicit system initiator; Organization and security domain; exact canonical container and closed resource set or container-inventory scope; provider installation/path/resource and reconciliation generation; one allowed operation class and closed call plan; activation and authorization consistency revisions; provider-enforcement and resource fences; compatibility profile; original deadline; immutable earliest expiry; and a persisted conservative whole-plan attempt/call/page/item/byte envelope.

The only eligible calls are declared pagination reads, snapshot reads, bounded verification reads, and compatibility-profile-declared safe retries of idempotent reads. Before each call and local outcome commit, WS-03 invokes the WS-06 read-only validator and enforces execution-local monotonic counters. Every dispatched or uncertain read consumes its local allowance. The operation performs zero durable reservation, permit, audit, or accounting writes per eligible call or page and finishes one logical audit with exact counts; interruption records the conservative reserved upper bound and an abandoned/crash outcome.
...
Process loss abandons the scope and conservatively consumes its remaining envelope. Recovery starts from the last trusted job cursor only after a fresh decision and new scope. A scope is never resumed, renewed, transferred, or extended by provider output, retry, restore, or an administrator.
...
- WS-03 owns compatibility profiles, provider calls, webhook verification, field classification, mappings, snapshots, scope references and whole-plan envelopes, execution-local accounting, projections, operation/quarantine state, and outcome selection.
...
| `T-ADR-0009-FULL-RECONCILIATION` | Every scope binding and bound has negative coverage; inventory cannot widen scope; process loss starts fresh; eligible pages write nothing; total PostgreSQL writes remain constant as pages grow. |
