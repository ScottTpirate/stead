...
A maintained profile that claims to map an external classification, handling, or regulatory regime must follow its authoritative implementing guidance and bind the exact source snapshot/version and digest, payload provenance, mapped scope/version, digest-bound tested coverage, and limitations into its signed activation material. A live URI is only a locator. The checked-in US-oriented file is `starter_reference` data and makes no authoritative mapping claim. For future mapping work, the [NARA CUI program](https://www.archives.gov/cui/about) defines CUI as unclassified information requiring safeguarding or dissemination controls, while the [NARA CUI Registry](https://www.archives.gov/cui/registry/category-marking-list) supplies category and marking data. [NIST SP 800-171 Rev. 3](https://csrc.nist.gov/pubs/sp/800/171/r3/final) supplies applicable protection requirements for CUI in nonfederal systems. These references do not make Stead accredited or turn a profile token into an official marking without an exact, reviewed mapping.
...
The profile portion of `B` uses only `stead.security-profile-rules.v1`: typed monotone implications, incompatibilities, sensitivity constraints, required non-empty dimensions, trusted-context requirements, and provenance-bound registry mappings. It cannot express an allow decision, remove a restriction, lower sensitivity, broaden an audience, perform I/O, invoke code, add an operator, or select behavior by profile ID. Unknown/dangling/duplicate IDs, cycles, contradictions, non-monotone implications, unsupported fields, stale mapping provenance, or digest failures reject activation.
...
after profile implication and dominance normalization. The sensitivity coordinate is totally ordered for every validated profile. The product is a partial order because compartments, controls, and audiences may be incomparable. Labels from different profile lines are incomparable unless a future separately signed, versioned, non-weakening bridge contract covers the exact source/target profile versions, direction, named operation, mapping semantics, trust binding, and proof obligations. A bridge is compatibility data for only those operations; it grants no resource access or data transfer. No such bridge ships in the initial release, and the v0.1 deployment-domain schema rejects every non-empty bridge list until a dedicated ADR and contract are approved.
...
Lowering sensitivity, removing a handling regime/category/compartment/control/export restriction, or broadening `releasable_to` is a lowering operation. It never occurs through ordinary join, migration, profile upgrade, or rollback. It is denied by default and requires the directive-mandated authority, source authority, written reason, audit, invalidation/reconciliation, and the stricter approval threshold and custody/separation rules selected by the active signed profile and deployment security-domain policy. The effective minimum is the maximum of the profile and deployment thresholds; distinctness and human-principal requirements are the conservative logical OR of both sources. The parameterized policy-output obligation records that effective result and both policy bases. Approval contracts remain principal-typed and do not assume that every reviewer is human; when the effective policy requires human separation of duty, it requires the configured number of distinct independently authenticated `user` principals. One principal acting through two identities or an Agent delegated by an approver does not satisfy separation of duty. No threshold is inferred from a profile name.
...
`CUI_BASIC` and `CUI_SPECIFIED` are illustrative handling-regime IDs in this starter, never sensitivity values. The closed starter rules require `UNCLASSIFIED`, a non-empty category dimension, and applicable trusted authority/handling context, but the category IDs do not claim an authoritative NARA mapping. A future maintained mapping must bind each category/subcategory to an exact reviewed registry snapshot and encode whether its governing authority specifies different handling; the display name cannot supply that fact. Neither regime globally dominates the other. Composition preserves applicable category-specific controls and applies generic requirements only as declared by the signed profile. A classified label cannot simultaneously satisfy this starter's CUI-handling sensitivity constraint.
...
| `SCH-DEPLOYMENT-DOMAIN` | Validate a profile-ID-keyed exact profile/version/ceiling map, trusted authorities, deployment-selected trust/cryptographic/assurance controls, and the v0.1 prohibition on non-empty bridge sets; a future bridge contract requires its own ADR | WS-06; WS-12 and WS-01 review; WS-13 and project owner approve |
...
- `T-ADR-0002-PROFILE-NEUTRALITY`: proves a synthetic third profile and a high-assurance non-government domain pass the same closed schema, typed monotone implication/incompatibility/context/mapping tables, signed activation, evaluator, profile-driven presentation, API, migration, and rollback seams without changing the canonical ontology, forking Stead or `stead-web`, adding profile-ID branches, or changing the browser/API architecture; executable-profile, user-policy-language, dangling/duplicate identifiers, cyclic/contradictory/non-monotone rules, URI-only external provenance, and UI-code-injection attempts fail; unknown/mismatched data remains fail closed; all v0.1 cross-profile operations and non-empty bridge sets deny.
- `T-ADR-0002-CUI-PROFILE`: Phase 3 may promote or supersede the limited US-oriented starter only with exact source snapshots/digests and tested mappings; CUI never appears as a sensitivity; Basic/Specified validation, non-dominance, category-specific control preservation, source/scope/version/limitation provenance, invalid classified-plus-CUI combinations, official-marking provenance, and no completeness/compliance/accreditation/FIPS claim. Phase 1 validates only the generic schema, starter limitations, and synthetic compatibility seam and does not complete an external vocabulary.
