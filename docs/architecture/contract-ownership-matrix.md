...
| `P-SCM-RECONCILIATION-GITEA-V1` field precedence, bounded-read scope consumption, reconciliation outcomes, and compatibility evidence | `/specs/provider-reconciliation/gitea-v1.yaml` | `WS-03` | `WS-02`, `WS-06`, `WS-07`, `WS-12`; approve `WS-01`, `WS-13`, project owner for the ADR-0009 locked-clause exception | Gitea; authorization/scm/core/worker/operations | `C-PROVIDER`; non-operative while ADR-0009 is Proposed; provider permission is enforcement, scope issuance/validation remains WS-06-owned, and ordinary UI reads remain provider-free |
