...
| `POL-DEPLOYMENT-DOMAIN` profile-qualified ceilings, trusted authorities, reserved bridge seam, integration/egress/storage/backup/runner and assurance controls | `/policies/deployment-domains/` | `WS-06` | `WS-12`; approve `WS-01`, `WS-13`, project owner | stead-api, stead-worker, steadctl, providers, deployment, release | `C-POLICY`; the profile-ID-keyed map makes duplicate/conflicting ceilings impossible; assurance and signature thresholds are data-driven; v0.1 rejects all non-empty bridge sets until a separately approved signed mapping/non-weakening contract exists |
