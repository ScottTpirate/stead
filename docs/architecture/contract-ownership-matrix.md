...
| `C-POLICY` | OpenFGA, OPA, label/lattice, security/deployment policy | `WS-06` alone edits executable security contracts. Domain policy owners propose inputs and review semantics; `WS-13` independently approves test coverage/mutation and bypass results; `WS-01` approves compatibility. Lowering or weakening policy also needs project owner and ADR when a locked decision is affected. |
...
| `SCH-TRUSTED-ATTRIBUTE` | `/packages/domain-schemas/identity/trusted-attribute/` | `WS-06` | `WS-01`; approve `WS-13` | identity, OpenFGA context, OPA | `C-POLICY`; authority/provenance/expiry mandatory |
| `SCH-DEPLOYMENT-DOMAIN` | `/packages/domain-schemas/security/deployment-domain/` | `WS-06` | `WS-12`, `WS-01`; approve `WS-13`, project owner | install config, OPA, providers, runners | `C-POLICY` |
| `SCH-PLATFORM-CONFIG` | `/packages/domain-schemas/config/` | `WS-12` | all provider owners, `WS-01`; approve `WS-06`, `WS-13` | platformctl/deploy/core/worker | `C-ARCH`; no secret values in rendered/exported schema |
...
| `API-MIGRATION` discover/dry-run/import/delta/cutover/report/redirect | `WS-01` | `WS-11` | `WS-06`, `WS-13` | web, platformctl | `C-PUBLIC` |
| `API-OPERATIONS` health/version/migration/config-safe-view/backup/restore/upgrade reports | `WS-01` | `WS-12` | `WS-06`, `WS-13` | platformctl, operator tooling | `C-PUBLIC`; secrets and protected data excluded |
| `API-EXPORT-IMPORT` portable organization/resource export/import | `WS-01` | `WS-10`, `WS-11`, `WS-12` | `WS-06`, `WS-13` | platformctl, external portability clients | `C-PUBLIC`; cross-domain/write-down remains denied |
...
| `P-IDENTITY-OIDC` human authentication provider | `/packages/provider-sdk/identity/oidc/` | `WS-06` | `WS-12`; approve `WS-01`, `WS-13` | identity-oidc; core/web/platformctl | `C-PROVIDER` |
...
| `POL-OPA-IO-V0.1` decision input/output, reason-safe result, bundle metadata | `/policies/opa/contracts/` | `WS-06` | all policy-input owners; approve `WS-01`, `WS-13`, project owner | core, provider gateways, platformctl, audit | `C-POLICY`; principal type is mandatory and future inputs reserve agent runtime, security domain, classification ceiling, compartment, model provider, tool scope, and execution environment; contract lands before Rego families |
| `POL-OPA-CLASSIFICATION` dominance, compartments/programs, dissemination/releasability, CUI regimes/categories, citizenship/organization restrictions | `/policies/opa/classification/` | `WS-06` | `WS-13`; approve `WS-01`, project owner | all protected operations | `C-POLICY`; 100% decision-table coverage |
| `POL-OPA-CONTEXT` auth strength, device, network/zone, session ceiling, time/expiry | `/policies/opa/context/` | `WS-06` | `WS-12`; approve `WS-01`, `WS-13` | all protected operations/provider paths | `C-POLICY` |
| `POL-OPA-FLOW` export/download/share, downgrade/write-down, cross-domain deny | `/policies/opa/data-flow/` | `WS-06` | `WS-10`, `WS-11`, `WS-12`; approve `WS-01`, `WS-13`, project owner | exports, blobs, migration, backups | `C-POLICY`; no built-in cross-domain allow |
| `POL-OPA-CI` runner, artifact, deployment, secret, action-catalog policy | `/policies/opa/ci/` | `WS-06` | `WS-09`; approve `WS-01`, `WS-13` | CI/runners/artifacts | `C-POLICY` |
| `POL-OPA-INFRA` infrastructure admission and deployment-domain policy | `/policies/opa/infrastructure/` | `WS-06` | `WS-12`; approve `WS-01`, `WS-13` | platformctl/Helm/admission integrations | `C-POLICY` |
| `POL-OPA-ORG-DENY` explicit organization denies | `/policies/opa/organization-deny/` | `WS-06` | `WS-02`; approve `WS-01`, `WS-13` | central authorization | `C-POLICY`; cannot redefine ontology/workflow |
| `POL-LABEL-LATTICE` join, inheritance, container ceiling, raise/downgrade | `/policies/security-label-profiles/lattice/` | `WS-06` | all container owners; approve `WS-01`, `WS-13`, project owner | domains, events, search, exports, providers | `C-POLICY`; property/mutation tests required |
| `POL-LABEL-COMMERCIAL` commercial sensitivity profile | `/policies/security-label-profiles/commercial/` | `WS-06` | `WS-01`; approve `WS-13`, project owner | standard installations | `C-POLICY`; signed/versioned bundle |
| `POL-LABEL-USGOV` US-government-oriented profile | `/policies/security-label-profiles/us-government/` | `WS-06` | `WS-12`, `WS-13`; approve `WS-01`, project owner | government profiles | `C-POLICY`; CUI distinct from classification, no certification claim |
| `POL-SCM-REPOSITORY` branch/review/check/signing/visibility/runner/retention declaration | `/modules/scm/contracts/repository-policy/` | `WS-03` | `WS-02`, `WS-09`; approve `WS-01`, `WS-06`, `WS-13` | organization/team/project policy reconciliation | `C-MODULE`; scope declarations cannot bypass OPA/OpenFGA |
...
All rows use the `EVT-CLOUDEVENT-BASE` envelope, the subject form `platform.<domain>.<action>.v<major>`, JSON Schema 2020-12, and AsyncAPI 3.1.x. `WS-07` is the sole editor of every event schema and channel; the named producer owner defines semantics by review rather than editing the shared contract.
...
| `EVT-ORGANIZATION` `platform.organization.*` and team changes | `WS-02` | `WS-03`, `WS-06`; approve `WS-13` | auth, scm, search, audit, notification | `C-EVENT` |
| `EVT-IDENTITY` `platform.identity.*` | `WS-06` | `WS-11`; approve `WS-13` | auth, audit, notification, migration | `C-EVENT` |
...
| `EVT-CLASSIFICATION` label/profile/attribute/ceiling/approval changes | `WS-06` | all projection/container owners; approve `WS-13`, project owner for downgrade semantics | search, notification, audit, providers, exports, caches | `C-EVENT`; invalidation/reconciliation acknowledgment required |
| `EVT-PROJECT` `platform.project.*`, initiative/cycle | `WS-02` | `WS-03`, `WS-04`; approve `WS-06`, `WS-13` | scm, knowledge, search, graph, activity | `C-EVENT` |
| `EVT-WORKITEM` `platform.workitem.*` and Work Item relationships | `WS-02` | `WS-03`; approve `WS-06`, `WS-13` | scm reconciliation, search, graph, activity, inbox | `C-EVENT` |
| `EVT-COMMENT` `platform.comment.*` across Work, Docs, and Pull Requests | `WS-02`, `WS-03`, `WS-04` through their owned domain operations | all three producers; approve `WS-06`, `WS-13` | activity, inbox, audit, search where enabled | `C-EVENT`; one canonical schema despite multiple provider/container origins |
| `EVT-KNOWLEDGE` `platform.document.*` and document review | `WS-04` | `WS-03`, `WS-10`; approve `WS-06`, `WS-13` | search, graph, activity, inbox, audit | `C-EVENT` |
...
| `migration.*` | job/stage/checkpoint/mapping/quarantine/redirect/report state | `WS-11` | migration port, platformctl/API | `C-DB`; resumable/idempotent and protected |
