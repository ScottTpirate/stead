...
The focused suites report 90.3% statement coverage for `internal/outbox` and 90.9% for `internal/transaction`. Compile-fail evidence contains 17 independently executed cases, including caller-supplied snapshotters, wrong invocation type, runtime-participant injection, separable validity checks, direct Session access, executor early-commit attempts, raw SQL/role/lifecycle access, and forged opaque values. Runtime tests mutate pointer, slice, and map owner views before `Execute`, preserve the backend/later-participant/deferred-intent bound value, reject shared or overlapping alias topology, reject derived contexts, and prove outer-scope and inner-binding escape, cancellation, deadline, panic, and missing-receipt paths wait before exactly one rollback. Proof-lifecycle tests add concurrent multi-mint contention, retained issuers, owner error/panic, invalid/outbox error and panic, failed/ambiguous commit error and panic, concurrent precommit use attempts, shared-copy invalidation, and exactly one postcommit activation for both revision and durable-effect receipts.
...
The provisional-proof and lifecycle-authority split was measured on 2026-09-02 against exact candidate `d8f44db6b707629ece49a81c2484e1d5241d5b99` with the same host, harness, sample count, and nearest-rank calculation. The extra closed-plan allocation is its distinct executor binding; finalization and boundary measurements additionally include shared atomic proof lifecycle state.

| Scenario | p50 | p95 | p99 | Allocation result | p50 delta from `d8f44db` |
|---|---:|---:|---:|---:|---:|
| reference-free two-participant plan plus outbox | 1,059 ns/op | 1,067 ns/op | 1,067 ns/op | 1,448 B/op, 23 allocs/op | +106.0 ns (+11.1%) |
| reference-bearing two-participant plan plus outbox | 9,388 ns/op | 9,906 ns/op | 9,906 ns/op | 4,809 B/op, 89 allocs/op | +31 ns (+0.3%) |
| final logical authorization/audit handoff, no intent | 861.6 ns/op | 869.3 ns/op | 869.3 ns/op | 856 B/op, 18 allocs/op | +186.2 ns (+27.6%) |
| bound-revision response recheck and release | 464.1 ns/op | 475.0 ns/op | 475.0 ns/op | 194 B/op, 6 allocs/op | +173.8 ns (+59.9%) |

The reference-bearing timing change is within microbenchmark sample noise. Against `d8f44db`, both plan paths add 48 bytes/one allocation for the executor binding and exact lifecycle-session association, finalization adds 80 bytes/one allocation, and the full revision-mint/recheck/release benchmark adds 48 bytes/one allocation. These remain internal seam measurements, not endpoint latency claims.
