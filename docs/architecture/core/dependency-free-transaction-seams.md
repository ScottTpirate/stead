...
Exact-session typed-port measurements were collected on 2026-08-30 on an Intel i9-12900H with Go's benchmark harness (`-benchtime=300ms -count=10`; nearest-rank percentiles over the ten aggregate `ns/op` samples). The delta compares this correction with parent `01996e7497599ebf934ae194e54ace59fb1d16a9`, the prior corrected candidate:

| Scenario | p50 | p95 | p99 | Allocation result | p50 delta |
|---|---:|---:|---:|---:|---:|
| two typed owner participants plus exact-receipt outbox slot | 696.2 ns/op | 716.4 ns/op | 716.4 ns/op | 880 B/op, 19 allocs/op | +66.5 ns (+10.6%) |
| final logical authorization/audit handoff, no intent | 581.7 ns/op | 603.5 ns/op | 603.5 ns/op | 576 B/op, 15 allocs/op | +42.9 ns (+8.0%) |
| bound-revision response recheck and release | 295.5 ns/op | 297.8 ns/op | 297.8 ns/op | 146 B/op, 5 allocs/op | -9.4 ns (-3.1%) |

The closed-plan p50 increase, 164 additional bytes, and four additional allocations are the measured interface-only price of backend/operation/call seals and exact typed ports; finalization adds 88 bytes and two allocations. These are not product endpoint/browser measurements and make no claim against PERF-002 latency. The endpoint/browser scenario is `not enabled`; response size is `0`; SQL queries/writes, OpenFGA/provider calls, NATS waits, browser requests, eager/lazy frontend bytes, and frontend chunk delta are all `0`.
