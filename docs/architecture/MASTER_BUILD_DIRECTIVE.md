...
Streams, content/downloads, exports, print, credential issuance, provider mutations, direct protocols, non-idempotent calls, operations that may outlive their logical request or job, and ambiguous external effects MUST use durable one-use effect controls with revocation/fencing, audit-before-effect, suppression, and reconciliation as applicable in both disclosure modes. On acceptance, ADR-0009's bounded internal provider-read scope is the sole provider-call exception: eligible pagination, snapshot, verification, and safe idempotent reads use one fresh central decision and one closed bounded logical call plan under CLS-003, read-only per-call scope/fence validation, and one logical audit instead of a durable permit transaction for each HTTP call. Ordinary finite metadata reads MAY use the deployment-selected request or commit boundary defined by CLS-003; browser and ordinary UI reads remain provider-free, and provider limitations MUST NOT bypass either path.
