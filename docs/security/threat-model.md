...
| TM-F005 | T, I | Label/permission change, ambiguous profile ceilings, or retained transport buffers leave stale projections, URLs, notifications, provider grants, or protected bytes | Closed profile semantics; profile-ID-keyed ceilings in every runtime context; ordered invalidation/reconciliation; DisclosureEgressFenceV1 delivery-or-purge proof; label-raise and crash/race tests | WS-06 with WS-02/03/07/08/10 | Critical | OPEN-P0 |
...
| TM-F018 | T, E, D | Unsigned, incompatible, profile-ID-special-cased, rule-injected, provenance-mismatched, or rolled-back OpenFGA/policy-decision/profile/schema/config weakens decisions or locks out service | Signed/versioned activation sets; closed typed monotone profile rules; exact source/evidence digests; empty v0.1 bridge set; model/policy/profile mutation tests; preflight; compatible rollback; before/after audit | WS-06 with WS-01/12/13 | Critical | OPEN-P0 |
...
| TM-F021 | D, E | OpenFGA/policy-decision/provider/egress-fence outage causes fail-open behavior, unsafe partial writes, or buffered post-revocation disclosure | Explicit timeout/error deny; transactional boundaries; authoritative transport-epoch fencing and terminal delivery-or-purge proof; durable-permit fallback only with equivalent suppression; degraded-mode contract; failure/chaos tests and recovery audit | WS-02/06 with WS-13 | Critical | OPEN-P0 |
