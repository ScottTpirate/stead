...
| OWGP representative resources | `node scripts/validate_owgp_examples.js` | 15/15 valid examples and four negative fixtures pass semantic checks, including closed label/entity fields, acting-principal restrictions, user/agent-only assignment, and audit/event context. |
| Cross-contract structure and references | `ruby scripts/validate_contracts.rb` | 18 primary machine documents parse; local JSON/YAML references resolve; fixed ontology, OpenAPI, AsyncAPI, provider, label/deployment, OPA, migration, and OpenFGA matrix assertions pass. |
